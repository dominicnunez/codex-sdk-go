...
There is also a deterministic same-behavior test in `appserver/protocol/dispatch_test.go`, so the described asynchronous-dispatch gap does not occur in the current code path.
