...
func TestWebSearchAction(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantType: "*protocol.SearchWebSearchAction",
		},
		{
			name:     "open page action",
			data:     `{"type":"openPage","url":"https://example.com"}`,
			wantType: "*protocol.OpenPageWebSearchAction",
		},
		{
			name:     "find in page action",
			data:     `{"type":"findInPage","url":"https://example.com","pattern":"test"}`,
			wantType: "*protocol.FindInPageWebSearchAction",
		},
		{
			name:     "other action",
			data:     `{"type":"other"}`,
			wantType: "*protocol.OtherWebSearchAction",
		},
	}
