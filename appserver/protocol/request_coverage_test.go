...
// defined in schema/json/ClientRequest.json has a corresponding service method on the Client,
...
func loadClientRequestMethods(t *testing.T) map[string]bool {
	...
	data, err := os.ReadFile(repoPath(t, "appserver", "protocol", "schema", "json", "ClientRequest.json"))
	if err != nil {
		t.Fatalf("ReadFile(schema/json/ClientRequest.json) failed: %v", err)
	}
	...
	if err := json.Unmarshal(data, &schema); err != nil {
		t.Fatalf("Unmarshal(schema/json/ClientRequest.json) failed: %v", err)
	}
	...
	if len(methods) == 0 {
		t.Fatal("found 0 client request methods in schema/json/ClientRequest.json")
	}
